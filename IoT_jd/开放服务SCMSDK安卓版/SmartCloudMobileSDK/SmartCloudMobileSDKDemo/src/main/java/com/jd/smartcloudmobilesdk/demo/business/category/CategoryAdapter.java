...
public class CategoryAdapter extends BaseRecyclerAdapter<Category> {
    ...
    protected RecyclerView.ViewHolder onCreateVH(ViewGroup parent, int viewType) {
        return new ViewHolder(getLayout(parent.getContext(), R.layout.item_category_list));
    }
    ...
    protected void onBindVH(RecyclerView.ViewHolder holder, int position, Category category) {
        if (holder instanceof ViewHolder) {
            ...
            contentHolder.nameView.setText(category.getCname());
            ImageLoader.getInstance().displayImage(category.getImg_url(), contentHolder.iconView);
        }
    }
